	...
	<HEAD>
		...
		<script type="text/javascript">
			...
			function findOrderInfoByOid(oid) {
				//清除上一次显示的的订单内容
				$("#showDivTab").html("");
				//在服务器发回响应数据之前显示loading.gif图片
				$("#loading").css("display", "block");
				
				//使用ajax发送异步请求
				$.post(
					"${pageContext.request.contextPath}/adminServlet?method=findOrderInfoByOid",	//请求的url地址
					{"oid" : oid},	//异步发送给服务器的请求数据
					function(data) {	//当服务器发回响应数据时执行此回调函数
						//回调函数得到执行说明已接收到服务器发回的响应数据，此时应该让loding.gif隐藏
						$("#loading").css("display", "none");
						var content = "";
						content += "<tr id='showTableTitle'><th width='20%'>图片</th><th width='25%'>商品</th><th width='20%'>价格</th><th width='15%'>数量</th><th width='20%'>小计</th></tr>";
						for(var i = 0; i < data.length; i ++) {
							content += "<tr style='text-align: center;'>" + 
											...
											"<td>3</td>" +
											"<td>" +
												"<span class='subtotal'>" + data[i].subtotal + "</span>" +
											"</td>" +
										"</tr>";
						}
						$("#showDivTab").html(content);
					},
					"json"	//以json格式解析服务器发回的响应数据
				);
				$("#shodDivOid").html(oid);
			}
        ...
        <div id="showDiv" class="blk" style="display:none;">
            <div class="main">
                ...
                <div id="loading" style="padding-top:30px;text-align: center;">
                	<img alt="" style="width: 102px; height: 136px;" src="${pageContext.request.contextPath }/images/loading.gif">
                </div>
