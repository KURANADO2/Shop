...
public class CallbackServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// 获得回调所有数据
		String p1_MerId = request.getParameter("p1_MerId");	//商户编号--商户在易宝支付系统的唯一身份标识
		...
		if (isValid) {
			...
			if (r9_BType.equals("1")) {
				// 为"1":浏览器重定向
				//修改订单状态为已付款（ 现在因为没有外网地址所以交易结果返回类型为"1",真正开发中交易结果返回类型为"2",所以真正开发中修改订单状态的代码应该放在if(r9_BType.equals("2")中)
				ProductService productServie = new ProductService();
				productServie.updateOrderState(r6_Order);
...
//				ProductService productServie = new ProductService();
//				productServie.updateOrderState(r6_Order);
				// 回复支付公司,支付成功必须回复易宝支付"success"，否则易宝支付将反复的调用callbackServlet知道收到响应"success"
